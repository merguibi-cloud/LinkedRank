...
import { linkedinInfluencers } from '../drizzle/schema.js';
...
async function cleanupDuplicates() {
  ...
  for (const [username, list] of duplicates) {
    ...
    console.log(`  ✅ Garde: ID ${keep.id} (${keep.followers} followers)`);
    
    for (const dup of toDelete) {
      console.log(`  ❌ Supprime: ID ${dup.id} (${dup.followers} followers)`);
  ...
  // Count remaining
  const remaining = await db.select({ count: sql`COUNT(*)` }).from(linkedinInfluencers);
